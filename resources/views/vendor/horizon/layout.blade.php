...
<head>
    ...
    {{-- Horizon's bundled Bootstrap theme bakes its purple accent (#7746ec / #8b5cf6)
         into compiled hex values rather than CSS custom properties, so it can't be
         retheme'd via a variable. These rules mirror the same selectors from
         vendor/laravel/horizon/dist/{styles,styles-dark}.css with the accent swapped
         to the app's blue-600, to match layouts/central.blade.php. Regenerate if the
         horizon/laravel package is upgraded and the accent stops matching. --}}
    <style>
        :root{--bs-primary: #2563eb;--bs-primary-rgb: 37, 99, 235}
        a{color:#2563eb}
        .form-control:focus{box-shadow:0 0 0 .25rem #2563eb40}
        .form-select:focus{box-shadow:0 0 0 .25rem #2563eb40}
        .form-check-input:focus{box-shadow:0 0 0 .25rem #2563eb40}
        .form-check-input:checked{background-color:#2563eb;border-color:#2563eb}
        .form-check-input[type=checkbox]:indeterminate{background-color:#2563eb;border-color:#2563eb}
        .form-range:focus::-webkit-slider-thumb{box-shadow:0 0 0 1px #111827,0 0 0 .25rem #2563eb40}
        .form-range:focus::-moz-range-thumb{box-shadow:0 0 0 1px #111827,0 0 0 .25rem #2563eb40}
        .form-range::-webkit-slider-thumb{background-color:#2563eb}
        .form-range::-moz-range-thumb{background-color:#2563eb}
        .btn-check:focus+.btn,.btn:focus{box-shadow:0 0 0 .25rem #2563eb40}
        .btn-primary{background-color:#2563eb;border-color:#2563eb}
        .btn-primary:disabled,.btn-primary.disabled{background-color:#2563eb;border-color:#2563eb}
        ...
        .btn-check:focus+.btn-outline-primary,.btn-outline-primary:focus{box-shadow:0 0 0 .25rem #2563eb80}
        .btn-check:checked+.btn-outline-primary,.btn-check:active+.btn-outline-primary,.btn-outline-primary:active,.btn-outline-primary.active,.btn-outline-primary.dropdown-toggle.show{background-color:#2563eb;border-color:#2563eb}
        .btn-check:checked+.btn-outline-primary:focus,.btn-check:active+.btn-outline-primary:focus,.btn-outline-primary:active:focus,.btn-outline-primary.active:focus,.btn-outline-primary.dropdown-toggle.show:focus{box-shadow:0 0 0 .25rem #2563eb80}
        .btn-outline-primary:disabled,.btn-outline-primary.disabled{color:#2563eb}
        .btn-link{color:#2563eb}
        .dropdown-item.active,.dropdown-item:active{background-color:#2563eb}
        .dropdown-menu-dark .dropdown-item.active,.dropdown-menu-dark .dropdown-item:active{background-color:#2563eb}
        .nav-link{color:#2563eb}
        .accordion-button:focus{box-shadow:0 0 0 .25rem #2563eb40}
        .page-link{color:#2563eb}
        .page-link:focus{box-shadow:0 0 0 .25rem #2563eb40}
        .page-item.active .page-link{background-color:#2563eb;border-color:#2563eb}
        .progress-bar{background-color:#2563eb}
        .list-group-item.active{background-color:#2563eb;border-color:#2563eb}
        .btn-close:focus{box-shadow:0 0 0 .25rem #2563eb40}
        .link-primary{color:#2563eb}
        .border-primary{border-color:#2563eb!important}
        .sidebar .nav-item a:hover,.sidebar .nav-item a.active{color:#2563eb}
        .sidebar .nav-item a.active svg{fill:#2563eb}
        .fill-primary{fill:#2563eb}
        .btn-muted.active{background:#2563eb}
    </style>

    {{-- The rest of the app has no dark theme, so pin Horizon to light rather than
         following the OS scheme — otherwise it flips to dark on its own and no
         longer matches the surrounding central-admin pages. The dark stylesheet
         (see Horizon::css() above) has no media restriction until the removed
         scheme-toggler component's mounted() hook would normally add one, so we
         disable it ourselves here instead. --}}
    <script>
        localStorage.setItem('horizonColorScheme', 'light');
        var horizonDarkStyle = document.querySelector('style[data-scheme="dark"]');
        if (horizonDarkStyle) horizonDarkStyle.media = 'max-width: 1px';
    </script>
</head>
<body>
<div class="app-topbar">
    <span class="app-topbar-brand">Central Admin</span>
    ...
    <a href="{{ url('/horizon') }}" class="active">Horizon</a>
